...
/// stab_rs module
#[pymodule]
fn linear(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
pub fn init_linear(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    let linear_child = PyModule::new_bound(_py, "stab_rs.linear")?;
    linear(_py, &linear_child)?;

    m.add("linear", &linear_child)?;

    _py.import_bound("sys")?
        .getattr("modules")?
        .set_item("stab_rs.linear", linear_child)?;
